...
const errCreatingPaidGroup = "Something went wrong when trying to create paid group chat."

func (svc *usecase) CreatePaidGroupChannel(ctx context.Context, req request.CreatePaidGroupChannel) (*response.PaidGroup, error) {
	...
	if user == nil {
		return nil, httperr.NewCtx(
			...
			http.StatusNotFound,
			errCreatingPaidGroup,
			fmt.Sprintf("CreatePaidGroupChannel returned nil user for request: %v", req),
		)
		...
		return nil, httperr.NewCtx(
			...
			"Only creators are allowed to create paid group channels.",
			"Only creators are allowed to create paid group channels.",
		)
	}
	...
	if marshalErr != nil {
		return nil, httperr.NewCtx(
			...
			http.StatusInternalServerError,
			errCreatingPaidGroup,
			fmt.Sprintf("Error marshaling paid group chat info during create: %v", marshalErr),
		)
	}
	...
	if marshalErr != nil {
		return nil, httperr.NewCtx(
			...
			http.StatusInternalServerError,
			errCreatingPaidGroup,
			fmt.Sprintf("Error marshaling channel data in CreatePaidGroupChannel: %v", marshalErr),
		)
	}
	...
	createGroupChatParams := sendbird.CreateGroupChannelParams{
		UserIDs:     userIDs,
		CustomType:  constants.ChannelTypePaidGroup,
		Name:        req.Name,
	...
	if createChannelErr != nil {
		return nil, httperr.NewCtx(
			...
			http.StatusInternalServerError,
			errCreatingPaidGroup,
			fmt.Sprintf("Error creating paid group chat for user %d: %v", user.ID, createChannelErr),
		)
	}
	...
	// Stripe protocol
	stripeProductName := fmt.Sprintf("paid-group-chat-%s", newGroupChannel.ChannelURL)
	...
	if createStripeProductErr != nil {
		return nil, httperr.NewCtx(
			...
			http.StatusInternalServerError,
			errCreatingPaidGroup,
			fmt.Sprintf("Error creating Stripe product for user %d: %v", user.ID, createStripeProductErr),
		)
	}

	insertedID, insertChatErr := svc.repo.UpsertPaidGroupChat(
		ctx,
		user.ID,
	...
	if insertChatErr != nil || insertedID == nil {
		return nil, httperr.NewCtx(
			...
			http.StatusInternalServerError,
			errCreatingPaidGroup,
			fmt.Sprintf("Error inserting paid group chat for user %d: %v", user.ID, insertChatErr),
		)
	}

	res := &response.PaidGroup{
		ID:                   *insertedID,
		GoatID:               user.ID,
		PriceInSmallestDenom: int64(req.PriceInSmallestDenom),
