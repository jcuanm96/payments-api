...
module "db" {
  ...
  db_name = "vama-staging-postgres"
}

module "cloud_build" {
  source       = "../../modules/CloudBuild"
  trigger_name = "staging-build-trigger"
  branch_regex = "^staging$"
...
module "cloud_run" {
  ...
  env = [
    ...
    { key = "REDIRECT_BASE_URL", value = google_app_engine_application.app.default_hostname },
    { key = "DYNAMIC_LINK_BASE_URL", value = "https://stagingvama.page.link/" },
    { key = "APPLE_BUNDLE_ID", value = "com.vama.app.staging" },
    ...
    { key = "REDIS_ENDPOINT", value = "redis-14137.c18569.us-central1-1.gcp.cloud.rlrcp.com" },
    { key = "REDIS_PORT", value = "14137" },
...
module "cloud_function" {
  ...
  bucketName          = "vama_health_checker_staging_bucket"
  functionName        = "vama_health_checker_staging"
  functionDescription = "Vama Health Checker Function for staging"
...
module "cloud_scheduler" {
  ...
  schedulerName        = "vama_health_scheduler_staging"
  schedulerDescription = "Vama Health Checker Scheduler for staging"
  httpMethod           = "POST"
...
resource "google_cloudbuild_trigger" "redirect-build-trigger" {
  name        = "redirect-build-trigger"
  description = "Build and deploy to App Engine service on push"
  ...
  substitutions = {
    _SERVICE_NAME   = "redirect"
    _API_BASE_URL   = var.api_base_url
    _IOS_URL_SCHEME = "staging.vama.me://"
  }
