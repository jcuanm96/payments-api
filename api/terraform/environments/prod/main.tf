...
module "db" {
  ...
  db_name = "vama-prod-postgres"
}

module "cloud_build" {
  source       = "../../modules/CloudBuild"
  trigger_name = "prod-build-trigger"
  branch_regex = "^main$"
...
module "cloud_run" {
  ...
  env = [
    ...
    { key = "REDIRECT_BASE_URL", value = google_app_engine_domain_mapping.default.domain_name },
    { key = "DYNAMIC_LINK_BASE_URL", value = "https://vamame.page.link/" },
    { key = "APPLE_BUNDLE_ID", value = "com.vama.app" },
    ...
    { key = "REDIS_ENDPOINT", value = "redis-17837.c18568.us-central1-1.gcp.cloud.rlrcp.com" },
    { key = "REDIS_PORT", value = "17837" },
...
module "cloud_function" {
  ...
  bucketName          = "vama_health_checker_prod_bucket"
  functionName        = "vama_health_checker_prod"
  functionDescription = "Vama Health Checker Function for prod"
...
module "cloud_scheduler" {
  ...
  schedulerName        = "vama_health_scheduler_prod"
  schedulerDescription = "Vama Health Checker Scheduler for prod"
  httpMethod           = "POST"
...
module "cloud_run_domain_mapping" {
